...
def print_results(error, size, cond_dict):
    ...
    print("Precision = %.2f" % (true_pos/(true_pos+false_pos)))
    print("Recall = %.2f" % (true_pos/(true_pos+false_neg)))
...
print("Training, CV, Test = %d, %d, %d" % (x_train.shape[0], x_cv.shape[0], x_test.shape[0]))
